...
class Day09 extends GenericDay {
  ...
  List<DiskEntry> parseInput() {
    ...
    return diskMap.split('').mapIndexed<DiskEntry>((index, element) {
  ...
  int solvePart1() {
    ...
    List<DiskEntry> getMovedDiskMap(List<DiskEntry> diskMap) {
      final cleanDiskMap =
          [...diskMap].removeTrailingFreeSpace()
            ..removeWhere((element) => element == const FreeSpace(0));
      ...
      while (true) {
        ...
        switch (freeSpace.blockLength - rightMostFile.blockLength) {
          ...
          case 0:
            ...
            cleanDiskMap.removeAt(rightMostFileIndex);
          case final diff when diff < 0:
            cleanDiskMap[freeIndex] = FileEntry(
              rightMostFile.id,
              freeSpace.blockLength,
            );
            cleanDiskMap[rightMostFileIndex] = FileEntry(
              rightMostFile.id,
              -diff,
            );
        }
    ...
    List<DiskEntry> getMovedDiskMap(List<DiskEntry> diskMap) {
      final cleanDiskMap =
          [...diskMap].removeTrailingFreeSpace()
            ..removeWhere((element) => element == const FreeSpace(0));
      var biggestFileId =
          (cleanDiskMap.lastWhere((element) => !element.isFreeSpace)
                  as FileEntry)
              .id;
      while (biggestFileId > 0) {
        final fileEntryIndex = cleanDiskMap.indexWhere((element) {
          switch (element) {
        ...
        final freeSpaceIndex = cleanDiskMap
            .take(fileEntryIndex)
            .toList()
            .indexWhere((element) {
...
class FileEntry extends DiskEntry {
  ...
  int checksum(int index) {
    return List<int>.generate(blockLength, (i) {
      return (i + index) * id;
    }, growable: false).fold(0, (previousValue, element) {
...
extension DiskMapX on List<DiskEntry> {
  ...
  int checksum() {
    final correctIndexed = mapIndexed<(int, DiskEntry)>((index, element) {
      final correctIndex = take(index).fold(0, (previousValue, diskEntry) {
        return previousValue + diskEntry.blockLength;
      });
      return (correctIndex, element);
    });
    return correctIndexed.fold(0, (previousValue, element) {
