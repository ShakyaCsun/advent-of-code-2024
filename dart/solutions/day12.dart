...
extension on Iterable<Position> {
    ...
    return fold(
      ...
      (previousValue, position) {
        return previousValue +
            4 -
            where(
              (element) => position.distanceSquare(element) == 1,
            ).length;
      },
    );
  }
  ...
  int numberOfSides(Field<String> field) {
    ...
    return Positions.adjacent.map<int>(
      (direction) {
        ...
        return pointsOnEdgeOfDirection.fold(
          0,
          (previousValue, edgePosition) {
